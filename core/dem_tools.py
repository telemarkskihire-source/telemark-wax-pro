...
def render_dem(T, ctx):
    ...
    with st.expander(T.get("dem_hdr","Esposizione & pendenza (DEM locale)"), expanded=False):
        try:
            dem = dem_patch(lat, lon)
            if not dem:
                st.warning(T.get("dem_err","DEM non disponibile ora. Riprova tra poco."))
                return
            sdeg, spct, adeg = slope_aspect_from_dem(dem["Z"], dem["spacing_m"])
            atxt = aspect_to_compass(adeg)
            c1,c2,c3 = st.columns(3)
            c1.metric(T.get("slope_deg","Pendenza (°)"), f"{sdeg:.1f}°")
            c2.metric(T.get("slope_pct","Pendenza (%)"), f"{int(round(spct))}%")
            c3.metric(T.get("aspect_dir","Esposizione (bussola)"), f"{atxt} ({adeg:.0f}°)")
        except Exception:
            st.warning(T.get("dem_err","DEM non disponibile ora. Riprova tra poco."))
