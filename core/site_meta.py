# core/site_meta.py
# Badge località + altitudine AUTO/MANUALE + DEM, con sync su click mappa
...
def render_site_meta(T: dict, ctx: dict) -> dict:
    """
    Mostra il badge (luogo/altitudine/TZ) e gestisce la logica:
    - AUTO: altitudine da get_elev() / DEM al cambio coordinate
    - MANUALE: numero modificabile e blocco dell'auto-sync fino a nuovo toggle
    Aggiorna anche slope/aspect (DEM 3x3) quando cambiano le coordinate.
    """
    lat = float(ctx["lat"])
    lon = float(ctx["lon"])
    ...
    # ricalcola DEM/pendenza/aspect solo quando cambia punto
    ...
    if coords_changed:
        try:
            ...
            if dem:
                ...
                st.session_state["slope_deg"] = None
                st.session_state["slope_pct"] = None
                st.session_state["aspect_deg"] = None
        ...
        # aggiorna altitudine automaticamente solo se non in manuale
        if mode != "manual":
    ...
    # Toggle manuale vs auto + input/metric
    ...
    # aggiorna ctx per gli altri moduli
    ctx.update(
        {
