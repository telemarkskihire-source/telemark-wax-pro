...
#     · si aggiorna SUBITO al click (via st.session_state[map_key]["last_clicked"])
#     · viene "agganciato" al punto più vicino di una pista downhill (snap)
# - Evidenzia la pista selezionata (colore diverso / più spessa)
# - Nomi piste SEMPRE visibili in mappa, con nome selezionato evidenziato
...
def render_map(T: Dict[str, str], ctx: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # inizializza marker per questo contesto la prima volta
    ...
    # selezione pista (persistente per contesto)
    ...
    # ------------------------------------------------------------------
    # 1) PRIMA di costruire la mappa: leggo l'ultimo click dalla state del widget
    ...
    # aggiorna ctx + session con questa posizione "grezza" (prima dello snap)
    ctx["lat"] = marker_lat
    ctx["lon"] = marker_lon
    ...
    if show_pistes:
        ...
        if had_click and polylines and click_lat is not None and click_lon is not None:
            snapped_lat, snapped_lon, idx, dist_m = _snap_to_nearest_piste_point(
                ...
                click_lon,
                polylines,
                max_snap_m=150.0,  # raggio più realistico per piste vicine
            )
        ...
        # se disattivo le piste, azzero la selezione per questo contesto
        selected_idx = None
        st.session_state[selected_piste_idx_key] = None
        ctx["selected_piste_distance_m"] = None
        ctx["selected_piste_name"] = None
