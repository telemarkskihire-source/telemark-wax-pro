...
# se in futuro vogliamo riattivare un filtro "neve", lo agganciamo qui
...
def _resample(points: List[Dict[str, float]], max_points: int) -> List[Dict[str, float]]:
    """Limita il numero di punti per non esplodere la query Mapbox (422)."""
    n = len(points)
    if n <= max_points:
        return points
...
def _build_path_param(points: List[Dict[str, float]]) -> str:
    ...
    pts = _resample(points, max_points=90)
...
def _fetch_frame(
    ...
    zoom 16.2 + pitch 72° → camera bassa, tipo elicottero/sciatore sopra la pista.
    """
    url = (
...
def _apply_snow_filter(img: Image.Image) -> Image.Image:
    """Attualmente disattivato; lasciamo hook per il futuro."""
    if not ENABLE_SNOW_FILTER:
        return img

    arr = np.asarray(img).astype("float32") / 255.0

    # leggerissimo aumento di contrasto e tono freddo (senza impastare).
...
def generate_pov_video(
    ...
    # path completo (per vedere tutta la pista dall'alto)
    path_param = _build_path_param(points)

    # timeline frame: ci muoviamo lungo la pista
    n_frames = int(duration_s * fps)
    # campioniamo tra il primo e il penultimo punto (usiamo i segmenti a→b)
