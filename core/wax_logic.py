...
def _current_level() -> str:
    """
    Legge il livello dallo stato impostato dal tuning dinamico:
    'tourist' / 'expert' / 'fis' / 'wc'
    """
    lvl = st.session_state.get("dyn_skier_level", "tourist")
    return str(lvl).lower()

def tune_for(Tsurf, discipline):
    """
    Restituisce (famiglia struttura, angolo spigolo, base bevel) in funzione di
    T_neve e DISCIPLINA, modulati per livello sciatore.
    Gli angoli SIDE sono angoli spigolo (87°, 88°, …), non bevel.
    """
    lvl = _current_level()  # tourist / expert / fis / wc

    # --- base "expert" (di riferimento) ---
    if Tsurf <= -10:
        fam = "Linear Fine"
        base0 = 0.7
        side_edge0 = {"SL":88.5,"GS":88.0,"SG":87.5,"DH":87.5}[discipline]
    elif Tsurf <= -3:
        fam = "Cross Hatch leggera"
        base0 = 0.8
        side_edge0 = {"SL":88.0,"GS":88.0,"SG":87.5,"DH":87.0}[discipline]
    else:
        fam = "Diagonal / V"
        base0 = 0.9 if Tsurf <= 0.5 else 1.1
        side_edge0 = {"SL":88.0,"GS":87.5,"SG":87.0,"DH":87.0}[discipline]

    # --- correzioni per livello ---
    if lvl in ("fis", "wc"):
        # più aggressivo: meno base, più bevel -> angolo spigolo leggermente più chiuso
        base = max(0.3, base0 - 0.2)
        side_edge = side_edge0 - 0.5
    elif lvl == "tourist":
        # molto permissivo: più base, spigolo un filo più "dritto"
        base = base0 + 0.3
        side_edge = side_edge0 + 0.5
    else:
        # expert (default)
        base = base0
        side_edge = side_edge0

    return fam, side_edge, base
...
def render_wax(T, ctx):
    ...
    for lbl, (s, e) in blocks:
        ...
        # tuning per disciplina — ORA dipende dal livello scelto nel tuning dinamico
        rows=[]
        for d in ["SL","GS","SG","DH"]:
            fam, side_edge, base = tune_for(t_med, d)
            rows.append((d, fam, f"{side_edge:.1f}°", f"{base:.1f}°"))
