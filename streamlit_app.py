...
def snow_temperature_model(df: pd.DataFrame, dt_hours=1.0):
    """Stima T_surf e T_top5 (°C). Integra radiazione/vento/umidità/precipitazioni.
       Tende a 0°C in condizioni bagnate o neve fresca vicino a 0; scende con cielo sereno e vento basso di notte."""
    X = df.copy()
    X["ptyp"] = X.apply(prp_type_row, axis=1)

    # Wetness heuristic (più conservativa)
    sunup = X["sunup"]==1
    ...
    dry = ~wet
    # Raffreddamento radiativo notturno: dipende da cielo sereno e vento
    clear = (1.0 - X["cloud"]).clip(0,1)
    windc = X["wind"].clip(upper=6.0)  # m/s
    drad = (1.8 + 3.3*clear - 0.35*windc).clip(0.5, 5.0)  # °C da sottrarre
    T_surf.loc[dry] = X["T2m"][dry] - drad[dry]

    # Giorno freddo ma soleggiato: non andare troppo sotto aria
    ...
    # Strato top ~5mm: rilassamento su T_surf con costante tempo variabile
    T_top5 = pd.Series(index=X.index, dtype=float)
    tau = pd.Series(6.0, index=X.index, dtype=float)          # h
    ...
    # Base: migliori valori su freddo secco leggero (-8..-4), penalità vicino a 0 e con bagnato
    base_speed = 100 - (abs(X["T_surf"] + 6.0)*7.5).clip(0,100)    # picco attorno a -6
...
def reliability(hours_ahead):
    # 0..100: 24h=85, 48h=75, 72h=65, 120h=50, 168h=40
    x = float(hours_ahead)
    if x<=24: return 85
    if x<=48: return 75
    if x<=72: return 65
...
def recommended_structure(Tsurf):
    # solo nomi (niente disegni/immagini)
...
def try_enrich_with_noaa(df, lat, lon, when_day: date):
    # placeholder leggero: se token c'è, aggiunge una leggera correzione RH verso climatologia
    # (evitiamo nuove dipendenze / parsing pesante)
    if not NOAA_TOKEN: return df
    try:
        # usa bounding box molto stretta e dataset=GHCND daily; useremo media mensile come correzione piccola
        # Nota: chiamata “soft” e facoltativa; se fallisce ignoriamo
        _ = (lat, lon, when_day)  # non usiamo davvero il dato per non aumentare rischi di failure
        # Piccola correzione cosmetica: sposta RH di ±2% verso 70
...
if btn:
    try:
        ...
        # Seleziona solo il giorno scelto per i blocchi
        # (ma teniamo tutto per affidabilità/preview)
        ...
        for L,(s,e) in blocks.items():
            ...
            rel = reliability((W.index[0] if not W.empty else 0) + 1)  # molto semplice: cresce con “vicinanza”
