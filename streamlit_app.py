# telemark_pro_app.py
# -------------------
# Telemark · Pro Wax & Tune (dark theme + ricerca con prefiltro Nazione)
# Algoritmo neve con "quick wins":
# - RH da T/td se manca
# - Wet-bulb (Stull)
# - Vento effettivo (log1p, 0–8 m/s)
# - Radiazione solare stimata (clear-sky semplice + correzione nuvolosità)
# - Albedo dinamico da età neve
...
def build_df(js, hours):
    ...
    df["time"] = pd.to_datetime(df["time"])                    # già nell'orario locale richiesto
    now0 = pd.Timestamp.now().tz_localize(None).floor("H")     # naive
    df = df[df["time"]>=now0].head(int(hours)).reset_index(drop=True)
    out = pd.DataFrame(index=df.index)
    out["time"] = df["time"]
    out["T2m"]  = df["temperature_2m"].astype(float)

    # RH (se manca la calcolo da T/td)
    if "relative_humidity_2m" in df:
        RH = df["relative_humidity_2m"].astype(float)
    else:
        td = df.get("dew_point_2m", df["temperature_2m"]).astype(float)
        # formula da T/td (Magnus-Tetens)
        a,b = 17.625, 243.04
        RH = 100*np.exp((a*td)/(b+td) - (a*out["T2m"])/(b+out["T2m"]))
    out["RH"] = RH.clip(1,100)
...
def wet_bulb_stull(T, RH):
    """
    Stull (2011) approximation, all Celsius, RH in %.
    ...
    T = T.astype(float)
    RHp = RH.astype(float)
    Tw = (T*np.arctan(0.151977*np.sqrt(RHp+8.313659))
          + np.arctan(T+RHp)
...
def solar_clear_approx(lat_deg, times):
    """SW_clear (W/m2) semplice: 990 * cosZ+ ."""
    # declinazione e ora solare (approssimata) — sufficiente per pesare i contributi
    lat = np.deg2rad(lat_deg)
    dayofyear = times.dt.dayofyear.values
    hour = times.dt.hour.values + times.dt.minute.values/60.0
    # declinazione (Cooper)
    dec = 23.44*np.pi/180.0 * np.sin(2*np.pi*(284+dayofyear)/365.0)
    # angolo orario rispetto a mezzogiorno (rad)
    h_ang = (hour-12.0) * (np.pi/12.0)
    cosZ = np.sin(lat)*np.sin(dec) + np.cos(lat)*np.cos(dec)*np.cos(h_ang)
    cosZ = np.maximum(cosZ, 0.0)
    return 990.0*cosZ

def dynamic_albedo(snowfall_series, T2m, times):
    """0.85 neve nuova → 0.55 neve vecchia, decresce con ore dall'ultima nevicata e T>0."""
    new_snow = snowfall_series.fillna(0) > 0.5
    # età neve (in ore) dal'ultima ora con nuova neve
    age = np.full(len(times), 999.0, dtype=float)
    last = -1
    for i, ns in enumerate(new_snow):
        if ns: last = i
        age[i] = 0 if last<0 else (i-last)
    base = 0.85*np.ones(len(times))
    decay = np.clip(age/48.0, 0, 1)  # dopo 48h tende al minimo
    alb = base - decay*(0.85-0.55)
    # se T>0 accelera l'invecchiamento (+0.1 verso 0.55)
    alb = np.where(T2m.values>0, np.maximum(alb-0.1, 0.55), alb)
    return pd.Series(alb, index=times.index)

# =============== Algoritmo Temperatura Neve & Scorrevolezza ===============
def snow_temperature_model(df: pd.DataFrame, lat):
    X = df.copy()
    ...
    # Vento effettivo (convettivo) — rendimenti decrescenti
    wind_eff = np.log1p(X["wind"].clip(0,8))  # ~0..~2.2
    X["wind_eff"] = wind_eff

    # Radiazione solare stimata + correzione nuvolosità
    SW_clear = solar_clear_approx(lat, X["time"])
    SW_down  = SW_clear * (1 - 0.75*(X["cloud"]**3))
    X["SW"]  = SW_down

    # Albedo dinamico
    X["albedo"] = dynamic_albedo(X["snowfall"], X["T2m"], X["time"])
    ...
    # Bilancio semplice (°C rispetto aria): raffreddamento radiativo notturno e perdita convettiva,
    # riscaldamento da SW*(1-albedo) nelle ore diurne
    clear = (1.0 - X["cloud"]).clip(0,1)
    noct_cool = (1.6 + 2.8*clear).clip(0.5, 4.5)            # °C
    conv_loss = 0.9*X["wind_eff"]                            # °C
    solar_gain = (X["SW"]*(1-X["albedo"]))/400.0             # scala in °C ~ 0..2.5

    delta = -noct_cool*(1-sunup.astype(int)) - conv_loss + (solar_gain*sunup.astype(int))
    T_surf = X["T2m"] + delta
    T_surf = np.where(wet, np.minimum(T_surf, 0.0), T_surf)   # mai >0 in bagnato; può stare sotto in secco

    # Smorzamento top 5 mm con costante di tempo variabile
    ...
    T_top5 = np.zeros(len(X))
    if len(X)>0:
        T_top5[0] = float(min(X["T2m"].iloc[0], 0.0))
        for i in range(1,len(X)):
            T_top5[i] = T_top5[i-1] + alpha.iloc[i]*(T_surf[i] - T_top5[i-1])

    X["T_surf"] = pd.Series(T_surf, index=X.index).round(2)
    X["T_top5"] = pd.Series(T_top5, index=X.index).round(2)

    # Indice di scorrevolezza (0-100): mix di prossimità a 0, umidità, radiazione e vento
    prox0 = 100 - (np.abs(X["T_surf"] + 0.5)*18).clip(0,100)   # massimo ~ -0.5°C
    hum   = (X["RH"]/100.0)*15                                 # più umido → più film
    sw    = (X["SW"]/800.0*20).clip(0,20)                      # sole contribuisce
    wpen  = (X["wind"].clip(0,10)/10.0*12)                     # troppo vento frena
    wet_pen = ((X["ptyp"].isin(["rain","mixed"])) | ((X["T_surf"]>-0.2) & (X["RH"]>90))).astype(int)*25
    speed_index = (prox0 + hum + sw - wpen - wet_pen).clip(0,100)
    X["speed_index"] = speed_index.round(0)
...
if btn:
    try:
        ...
        # Modello neve (usa lat per radiazione)
        ...
        for L,(s,e) in blocks.items():
            ...
            # Filtro sul giorno scelto
            day_df = res[res["time"].dt.date == target_day].copy()
            if day_df.empty:
                W = res.head(7).copy()
            else:
            ...
            # affidabilità grezza: più vicino = maggiore
            h_ahead = (W.index[0] if not W.empty else 0) + 1
            rel = reliability(h_ahead)

            # Banner condizioni
            st.markdown(
                f"<div class='banner'><b>Condizioni previste:</b> {k} · "
                f"<b>T_neve med</b> {t_med:.1f}°C · "
