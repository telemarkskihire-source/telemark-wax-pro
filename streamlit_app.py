...
# ------------------------ PAGE & THEME (sfondo chiaro) ------------------------
PRIMARY = "#0ea5b7"   # turchese Telemark più visibile su bianco
...
h1,h2,h3,h4,h5 {{ color:{ACCENT}; }}
label, p, span, div {{ color:{ACCENT}; }}
.badge {{
  border:1px solid #e5e7eb; padding:6px 10px; border-radius:999px;
  font-size:.78rem; color:{MUTED};
}}
.card {{
  background:{CARD_BG}; border:1px solid #e5e7eb; border-radius:16px;
...
.kpi .lab {{ font-size:.78rem; color:#0284c7; }}
.kpi .val {{ font-size:1rem; font-weight:800; color:{ACCENT}; }}
hr {{ border-color:#e5e7eb }}
.stButton>button {{
  background:{PRIMARY}; color:white; border:none; font-weight:700; border-radius:12px;
}}
</style>
...
def concise_label_from_nominatim(item: dict) -> str:
    """Costruisce etichetta breve stile Meteoblue: '🇮🇹 Champoluc, AO, IT'."""
    addr = item.get("address", {}) or {}
    cc = (addr.get("country_code") or "").upper()
    flag = flag_emoji(cc)
    # preferisci city/town/village/hamlet
    name = addr.get("city") or addr.get("town") or addr.get("village") or addr.get("hamlet") \
           or item.get("name") or item.get("display_name", "").split(",")[0]
    # provincia/codice breve (IT: state_district o county → sigla se presente)
    admin = addr.get("state_district") or addr.get("county") or addr.get("state") or ""
    # compattiamo admin (es. 'Aosta' -> 'AO', 'Torino' -> 'TO') se possibile
    abbrev_map = {
        "Aosta": "AO","Torino":"TO","Vercelli":"VC","Biella":"BI","Verbano-Cusio-Ossola":"VB",
        "Valle d'Aosta": "AO","Valle d’Aosta":"AO"
    }
    admin_short = abbrev_map.get(admin, admin.split()[0][:3].upper() if admin else "")
    parts = [p for p in [name, admin_short if admin_short else None, cc] if p]
    return f"{flag}  {', '.join(parts)}"

# ------------------------ RICERCA LOCALITÀ (live, no timezone in UI) ------------------------
def nominatim_search(search: str):
    ...
    try:
        ...
        for item in r.json():
            ...
            label_short = concise_label_from_nominatim(item)
            key = f"{label_short}|||{lat:.6f},{lon:.6f}"
            st.session_state._geo_map[key] = (lat, lon, label_short)
            out.append(key)  # st_searchbox mostra questo testo; noi mappiamo poi su _geo_map
        return out
    except Exception:
        return []
...
def window_slice(res, s, e):
    t = pd.to_datetime(res["time"]).dt.tz_localize(tz.gettz("Europe/Rome"), nonexistent='shift_forward', ambiguous='NaT')
    D = res.copy(); D["dt"] = t
    today = pd.Timestamp.now(tz=tz.gettz("Europe/Rome")).date()
    W = D[(D["dt"].dt.date==today) & (D["dt"].dt.time>=s) & (D["dt"].dt.time<=e)]
    return W if not W.empty else D.head(7)

# ------------------------ WAX (8 marchi non-fluoro) ------------------------
...
# ------------------------ STRUTTURA & LAME (stile Wintersteiger) ------------------------
def tune_for(t_surf, discipline):
    ...
    if t_surf <= -10:
        ...
        fam = ("cross","Incrociata universale / leggera onda")
        base = 0.7; side_map = {"SL":88.0, "GS":88.0, "SG":87.5, "DH":87.0}
    else:
        fam = ("chevron","Chevron/V con scarico (umido/caldo)")
        base = 0.8 if t_surf <= 0.5 else 1.0
        side_map = {"SL":88.0, "GS":87.5, "SG":87.0, "DH":87.0}
    return fam, side_map.get(discipline, 88.0), base

def draw_structure(kind: str, title: str):
    """Preview pulita tipo Wintersteiger (niente colori strani, gole nette)."""
    fig = plt.figure(figsize=(3.6, 2.0), dpi=180)
    ax = plt.gca()
    ax.set_facecolor("#dcdfe3")       # base soletta
    ax.set_xlim(0, 100); ax.set_ylim(0, 60); ax.axis('off')
    groove = "#2b2b2b"

    if kind == "linear":
        # righe verticali regolari, passo fine
        for x in range(6, 100, 5):
            ax.plot([x, x], [6, 54], color=groove, linewidth=2.6, solid_capstyle="round")
    elif kind == "cross":
        # incroci simmetrici (≈ rettilineo 45°)
        for x in range(-10, 120, 10):
            ax.plot([x, x+55], [6, 54], color=groove, linewidth=2.2, alpha=0.95)
        for x in range(10, 110, 10):
            ax.plot([x, x-55], [6, 54], color=groove, linewidth=2.2, alpha=0.95)
    elif kind == "chevron":
        # chevron verso centro per scarico laterale
        ...
        # diagonale unica (variante calda a scarico)
        for x in range(-20, 120, 8):
            ax.plot([x, x+50], [6, 54], color=groove, linewidth=3.0, alpha=0.95)
        ...
        for L,(s,e) in {"A":(A_start,A_end),"B":(B_start,B_end),"C":(C_start,C_end)}.items():
            ...
            for i,(brand,col,bands) in enumerate(BRAND_BANDS[:4]):
                ...
                cols[i].markdown(
                    ...
                    f"<div style='font-weight:800;color:{ACCENT}'>{rec}</div></div></div>",
                    unsafe_allow_html=True
                )
            for i,(brand,col,bands) in enumerate(BRAND_BANDS[4:]):
                rec = pick(bands, t_med)
            ...
            # Discipline pre-selezionate tutte
