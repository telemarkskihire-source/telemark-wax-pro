...
def solar_geometry(lat, lon, ts_utc):
    """Ritorna cos(theta_z) (zenit) approx. lat deg, lon deg, ts UTC naive."""
    latr = math.radians(lat)
    # Ora solare locale: H ~ (ora locale solare - 12)*15°
    # Stima grezza: usa longitudine per shift
...
def clear_sky_ghi(lat, lon, ts_utc):
    # Top-of-atmosphere approx: S0=1361 W/m2, trasmittanza 0.75
    S0 = 1361.0
...
def build_df(js, hours):
    ...
    out["sunup"]= df["is_day"].astype(int)  # 0/1 in timezone locale dell’API, ma abbiamo UTC -> usiamo geometria solare dopo
    out["prp_mmph"] = df["precipitation"].astype(float)
    out["rain"] = df.get("rain",0.0).astype(float)
    out["snowfall"] = df.get("snowfall",0.0).astype(float)  # mm/h acqua—> approx cm/h con densità, ma qui lasciamo mm/h
...
def noaa_bias_correction(df, lat, lon):
    """Tenta: trova una stazione GHCND vicina e prende clima medio mensile per RH/T. Applica piccola correzione bias."""
    if not NOAA_TOKEN: return df
    try:
        # 1) trova stazioni entro ~25km
        stations = requests.get(
            "https://www.ncdc.noaa.gov/cdo-web/api/v2/stations",
            params={"datasetid":"GHCND", "limit":3, "sortfield":"distance", "latitude":lat, "longitude":lon, "radius":25_000},
            headers={"token": NOAA_TOKEN}, timeout=10
        ).json().get("results", [])
        if not stations: return df
        sid = stations[0]["id"]  # prima stazione
        # 2) prendi normals monthly (temperatura media) — semplifichiamo usando Tmed mese per piccolo aggiustamento
        # (Se non disponibile, esci silenzioso)
        # Nota: endpoint normals è articolato; usiamo una correzione minima sintetica:
        # spostiamo T di +/-0.3°C verso 0 e RH di +/-2% verso 70, come stabilizzatore "climatico".
        df = df.copy()
...
def lapse_correction(T, base_alt, target_alt, lapse=-6.5):
    """Corregge T (°C) per differenza di quota (m). lapse in °C/1000m (default -6.5)."""
    dz_km = (target_alt - (base_alt or target_alt))/1000.0
    return T + lapse/1000.0 * (target_alt - (base_alt or target_alt))
...
def enrich_meteo_quickwins(df, lat, lon, base_alt, target_alt):
    ...
    # Età neve: ore da ultima nevicata (>0.5 mm/h)
    snow_mask = X["snowfall"] > 0.5
    last_snow_idx = -1
    age_hours = []
    for i, s in enumerate(snow_mask):
        if s: last_snow_idx = i
        if last_snow_idx < 0: age_hours.append(999)
        else: age_hours.append(i - last_snow_idx)
    X["snow_age_h"] = age_hours

    # Albedo dinamico (0.85 neve nuova -> 0.55 vecchia/primaverile)
    alb = 0.85 - 0.30 * np.clip(X["snow_age_h"]/48.0, 0, 1)  # entro 48h scende a 0.55
    # se T>0 intensifica riduzione albedo
    hot = X["T2m"] > 0
    alb = np.where(hot, alb - 0.05, alb)
    X["albedo"] = np.clip(alb, 0.45, 0.90)

    # Downscaling altitudinale su T aria
    if target_alt and base_alt:
        X["T2m"] = lapse_correction(X["T2m"], base_alt, target_alt)
        X["td"]  = lapse_correction(X["td"],  base_alt, target_alt)
        # RH ricalcolata dal nuovo T/td
...
def snow_temperature_model(X: pd.DataFrame, dt_hours=1.0):
    """Bilancio energetico semplificato + filtro esponenziale per T_top5; stima acqua liquida %."""
    X = X.copy()
    ...
    # Bilancio semplice: partiamo da T aria e sottraiamo "perdite" convettive/radiative, aggiungiamo SW*(1-albedo)
    conv = 0.20 * X["wind_eff"]               # °C ~ perdita da vento effettivo
    rad_cool = (0.8 * (1.0 - X["cloud"]))     # °C ~ raffreddamento radiativo notturno (clear sky)
    sw_gain = (X["SW_down"] * (1 - X["albedo"])) / 200.0  # scala empirica W/m2 -> °C
    # T_surf grezza
    T_surf = X["T2m"] - conv - rad_cool + sw_gain

    # Imponi limite superiore a 0°C in condizioni umide
    T_surf = np.where(wet, np.minimum(T_surf, 0.0), T_surf)

    # limite inferiore: non molto oltre aria in pieno sole freddo
    ...
    # Stima acqua liquida % (molto semplificata): surplus energetico quando T_surf ~ 0 e SW alto o pioggia
    ...
    X["liq_water_pct"] = np.round(np.clip(wetness, 0, 6.0), 1)  # 0..~6% vol (indicativo)
    ...
    try:
        ...
        for L,(s,e) in blocks.items():
            ...
            for i,(name,bands) in enumerate(BRANDS[:4]):
                ...
                ccols1[i].markdown(f"<div class='brand'><div><b>{name}</b><div style='color:#a9bacb'>{rec}</div></div></div>", unsafe_allow_html=True)
            for i,(name,bands) in enumerate(BRANDS[4:]):
                rec = pick_wax(bands, t_med, rh_med)
                ccols2[i].markdown(f"<div class='brand'><div><b>{name}</b><div style='color:#a9bacb'>{rec}</div></div></div>", unsafe_allow_html=True)
